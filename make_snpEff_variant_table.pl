...
print "Position\tTranscript_ID\tSnpEff\n";
  ...
  foreach my $a (@attributes) {
    if ($a =~ /Variant_effect=/) {
      ...
      foreach (@annos) {
        ...
        # I don't want to include gene names in transcript ID list.
        # Examples:
        # gene_variant 0 gene WASH7P
        # intragenic_variant C gene OR4G11P
        # intergenic_variant T intergenic_region MTND2P28-GENE_cds2
        # upstream_gene_variant A gene TUBB8P11
        shift @ann for 1..3;
        foreach (@ann) {
          if ($_ =~ /[NMRXP]{2}_/) { 
